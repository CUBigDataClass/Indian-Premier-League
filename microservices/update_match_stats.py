...
def update_player_info(player_id_map, pub_json, deliveries_info_response, player_type):
    try:
        if deliveries_info_response['found']:
            ...
            if 'total_extras' not in player_info:
                player_info['total_extras'] = 0
            ...
            if 'total_extras' not in player_match_info:
                player_match_info['total_extras'] = 0
            ...
            if (player_type == 'bowler'):
                ...
                if pub_json['extra_runs'] != 0:
                    player_info['total_extras'] += pub_json['extra_runs']
                    player_match_info['total_extras'] += pub_json['extra_runs']
...
def lambda_handler(event, context):
    ...
    for each_ in event['Records']:
        ...
        if (action == 'INSERT' or action == 'MODIFY'):
            ...
            # print(player_info_1)
            # print(player_info_2)
            insert_into_es(player_info_1['_source'], es_config['es_players_index'], pid_1)
            insert_into_es(player_info_2['_source'], es_config['es_players_index'], pid_2)
